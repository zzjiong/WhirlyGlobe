...
bool MapboxVectorTileParser::parse(PlatformThreadInfo *styleInst,RawData *rawData,VectorTileData *tileData)
{
    ...
    const double sx = tileSize / (tileData->bbox.ur().x() - tileData->bbox.ll().x());
    const double sy = tileSize / (tileData->bbox.ur().y() - tileData->bbox.ll().y());
    //Tile origin is upper left corner, in epsg:3785
    const double tileOriginX = tileData->bbox.ll().x();
    const double tileOriginY = tileData->bbox.ur().y();
